...
impl<'a> codespan_reporting::files::Files<'a> for SourceMap {
    ...
    fn line_index(&'a self, id: Self::FileId, byte_index: usize) -> Result<usize, codespan_reporting::files::Error> {
        let file = self.files.get(id.0 as usize).ok_or(codespan_reporting::files::Error::FileMissing)?;
        Ok(file.line_starts.binary_search(&byte_index).unwrap_or_else(|x| x - 1))
    }

    fn line_range(&'a self, id: Self::FileId, line_index: usize) -> Result<std::ops::Range<usize>, codespan_reporting::files::Error> {
        let file = self.files.get(id.0 as usize).ok_or(codespan_reporting::files::Error::FileMissing)?;
        let start = *file.line_starts.get(line_index).ok_or(codespan_reporting::files::Error::LineTooLarge { 
            given: line_index,
            max: file.line_starts.len(),
        })?;
        let end = file.line_starts.get(line_index + 1).cloned().unwrap_or(file.source.len());
        Ok(start..end)
    }
}
