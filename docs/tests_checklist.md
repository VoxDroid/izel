...
- [ ] **Numeric Boundaries**:
    ...
    - [ ] `0xG` (invalid hex - should report error, not panic).
...
- [ ] **Error Recovery**:
    ...
    - [ ] Missing semicolon in a sequence.
...
- [ ] **Movement**:
    - [ ] Use of a value after it was passed by-value to a `forge`.
    - [ ] Movement out of a `shape` field.
- [ ] **References**:
    - [ ] Creating a `&~` (mutable borrow) while a `&` (immutable) exists.
...
### 3.1 Witness Proofs
- [ ] **Construction Gating**:
    - [ ] Attempt to manually construct `Witness::new()` outside a `@proof` function.
- [ ] **Safety Bypass**:
...
- [ ] **Pre-condition Violations**:
    - [ ] Call `factorial(-1)` with a constant (compile error).
    - [ ] Call `factorial(n)` where `n < 0` at runtime (runtime panic).
- [ ] **Post-condition Verification**:
    - [ ] A function that returns a value outside its `@ensures` range should fail verification.

### 3.3 Memory Zone Isolation
- [ ] **Leak Test**: Allocate 1,000,000 small objects in a `zone` and monitor RSS after the block ends.
- [ ] **Escape Test**:
    - [ ] Assign a reference to a zone-allocated `String` to a variable declared outside the `zone`.
...
### 5.2 `izel-fmt`
- [ ] **Idempotency**: `izel fmt` run twice on the same file should change 0 bytes.
